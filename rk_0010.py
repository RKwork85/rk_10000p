...
if params['gradient_accumulation_steps'] in gradient_accumulation_steps_box:
    gradient_accumulation_steps_score = 1   
...
group_args_score = 3 if is_new_case_in_cases(group_args, groupA_args) else 0
print(group_args_score)
if group_args_score == 3:
    flag = True
else:
    flag = False
...
total_score = (dataset_test_ratio_score + batch_size_score + num_train_epochs_score + gradient_accumulation_steps_score + eval_steps_score + group_args_score + global_step_score) * 1.2

print("选手最终得分：", total_score)


'''
统计前还需要做判断：有没有统计的资格：只有当选手所有参数都设置正确才进入统计阶段： 记录一个表：选手编号： 220401 ，是否进入统计： False/True, 参数设置： 把选手设置的参数给记录下来
还需要有一个统计：统计所有选手best_step中：loss值和acc率的排行
应该是一个单独的程序：

'''
eval_loss = params[-3]['eval_loss']
eval_acc = params[-3]['eval_acc']

print("eval_acc",eval_acc,"eval_loss",eval_loss)
